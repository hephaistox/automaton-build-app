...
(def registry-schema
  [:map-of :symbol
   [:map {:closed true} [:doc :string] [:pf {:optional true} :keyword] [:task-fn :symbol]
    [:la-test [:map {:closed true} [:skip? {:optional true} :boolean] [:process-opts {:optional true} :map] [:cmd [:vector :string]]]]]])

(when (nil? (build-schema/valid? registry-schema registry)) (build-log/error "The bb task registry does not comply the schema"))
...
(defn update-bb-tasks
  ...
  * `exclude-tasks` set of tasks to exclude"
  ...
  (let [bb-tasks (map symbol bb-tasks)
    ...
    (when (bb-edn/update-bb-edn dir (partial update-bb-tasks* registry-bb-tasks exclude-tasks))
