...
(defn exec
  ...
  [_task-map {:keys [app-dir publication message] :as _app-data}]
  (let [{:keys [address branch major-version]} publication]
    (when (build-files/is-existing-file? "version.edn") (println (format "Current version is `%s`" (build-edn-utils/read-edn "version.edn"))))
