(ns automaton-build-app.tasks.reports
  (:require [automaton-build-app.app :as build-app]
            ...
            [automaton-build-app.code-helpers.analyze.forbidden-words :as
...
(defn keyword-report
  [[build-data clj-repo]]
  (let [filename
          (get-in build-data [:doc :reports :output-files :forbidden-words] #{})
        forbidden-words (get-in build-data [:doc :reports :forbidden-words] #{})
        regexp (build-forbidden-words/coll-to-alternate-in-regexp
                 forbidden-words)
