...
(defn remove-nodes
  ...
  ```"
  (:require [automaton-build-app.data-structure.graph.graph-impl :as graph-impl]
  ...
  * `graph` the graph to explore"
  ...
  (loop [graph-to-process graph
    ...
    (let [[nodes-with-no-successor updated-graph]
      ...
      (cond (or (empty? nodes-with-no-successor) (neg? nb-nodes)) (do (build-log/error "Cycle found in the graph")
