    ...
    (let [header-id "margin-header"
      ...
      (when pdf-path
        ...
        (build-pdf/html-str->pdf {:html-str html-str
                                  ...
                                  :margin-box {:margin-box {:top-left {:element header-id}
...
(defn configuration-data-by-language-to-html-pdf
  ...
  [{:keys [metadata filename]
    :as blog-data}]
  (let [{:keys [title description keywords]
         :or {description (str "Hephaistox document named: " filename)}}
        metadata
        keywords (str/join "," (concat ["hephaistox" "supply" "chain" "it"] (when keywords (str "," keywords))))]
    (create-html-pdf
     (assoc (select-keys blog-data [:md-path :pdf-path :html-path :title]) :pdf-metadata (pdf-metadata title description keywords)))))
  ...
  (->> (build-edn-utils/read-edn blog-config-file)
       (map (fn [[lang
              ...
              (assoc configuration-data
                     ...
                     :html-path (build-files/create-file-path output-html-dir (str filename ".html")))))))
  ...
  (let [blog-config-files (build-files/search-files customer-materials-dir "**.edn")]
    ...
    (if (empty? blog-config-files)
      ...
      (do (doseq [blog-config-file blog-config-files]
            ...
            (let [configuration-data (configuration-data blog-config-file output-html-dir output-pdf-dir)]
              (doseq [{:keys [pdf-path html-path md-path]} configuration-data]
                (if (and (build-files/modified-since blog-config-file [pdf-path html-path])
                  ...
                  (do (build-log/debug-format "Blog file `%s` and `%s` are regenerated from `%s`" pdf-path html-path md-path)
                      (doseq [configuration-data-by-language configuration-data]
                        (configuration-data-by-language-to-html-pdf configuration-data-by-language)))))))
          true))))
