...
(defrecord GithubAction [app-name container-dir app-dir remote-repo-account tag]
  build-containers/Container
    ...
    (build [this publish?]
      (let [app-files-to-copy-in-cc-container (map (partial build-files/create-file-path app-dir) [build-deps-edn/deps-edn "package.json"])
            image-name (build-containers/container-tagged-name this)]
        (build-log/debug-format "Create github-action container image `%s` for cust-app `%s`" image-name app-name)
        (build-local-engine/build-and-push-image image-name
                                                 remote-repo-account
                                                 container-dir
    ...
    (connect [this]
      (if (build-containers/build this false)
        (build-local-engine/container-interactive (build-containers/container-tagged-name this) app-dir)
        (build-log/warn "Connection to the container is skipped as build has failed"))))
