...
(defrecord GithubAction
  ...
  (build [this publish?]
    (let [app-files-to-copy-in-cc-container (map (partial build-files/create-file-path app-dir)
                                                 [build-deps-edn/deps-edn "package.json" "shadow-cljs.edn"])
          image-name (build-containers/container-name this)]
      (build-log/debug-format "Create githubaction container image `%s` for cust-app `%s`" image-name app-name)
  ...
  (connect [this]
    (if (build-containers/build this false)
      (build-local-engine/container-interactive (build-containers/container-name this)
                                                app-dir)
