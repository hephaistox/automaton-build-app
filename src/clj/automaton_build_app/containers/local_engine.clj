...
(defn container-installed?*
  ...
  * `docker-cmd` optional parameter telling the container command"
  ([docker-cmd]
   (if (build-cmds/execute-silently [docker-cmd "-v" {:out :string
                                                      :dir "."}])
     true
     (do
       (build-log/error "Docker is not properly installed")
       false)))
  ([]
...
(defn build-container-image
  ...
  * `target-container-dir` is where the Dockerfile should be"
  ...
  (when (build-cmds/execute-and-trace ["docker" "build" "--platform" "linux/amd64" "-t" container-image-name "." {:dir target-container-dir}])
...
(defn container-interactive
  ...
  * `container-local-root` is the local directory where the `/usr/app` in the container will be connected"
  ...
  (build-cmds/execute-and-trace ["docker" "run" "--platform" "linux/amd64" "-p" "8282:8080" "-it" "--entrypoint" "/bin/bash"
                                "-v" (str (build-files/absolutize container-local-root)  ":/usr/app") container-image-name {:dir "."}]))
...
(defn build-and-push-image
  ...
  * `publish?` do the publication if true, skip otherwise"
  ...
  (when (container-installed?)
    ...
    (let [files (keep build-files/is-existing-file?
                      files)]
      (build-files/copy-files-or-dir (concat [image-src-dir]
                                             files)
                                     assembled-container-dir)
      (when (build-container-image image-to-build
                                   assembled-container-dir)
        (if publish?
