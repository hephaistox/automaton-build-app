...
(deftest execute-report-test
  (let [clj-repo (build-clj-code/->CljCodeFileRepo {"foo.clj" ["  automaton-foobar"]
    ...
    (testing "Comment report is returning expected lines"
      (is (= [["foo.clj" ["automaton-foobar"] "automaton-foobar"] ["foo2.clj" ["automaton_foobar"] "automaton_foobar"]]
             (sut/forbidden-words-matches (sut/coll-to-alternate-in-regexp [#"automaton[-_]foobar" #"automaton[-_]foobar"]) clj-repo))))))
