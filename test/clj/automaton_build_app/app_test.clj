...
(def app-stub
  ...
  {:monorepo {:app-dir "app_stub"},
   ...
   :customer-materials
     {:html-dir "tmp/html", :dir "customer_materials", :pdf-dir "tmp/pdf"},
   :doc {:dir "docs",
         ...
         :code-doc {:dir "docs/code",
                    ...
                    :description
                      "Autonomous project to build an hephaistox app"}},
...
(deftest build-app-test
  (testing "Current project should generate its map"
    (is (< 0 (count (@sut/build-app-data_ ""))))))
...
(deftest get-cljc-s-src-dirs-test
  (testing "automaton-buil-app has no cljc or cljs files"
    (is (= 0 (count (sut/cljs-compiler-classpaths built-app-stub))))))

(deftest get-clj-c-s-src-dirs-test
  (testing "Returns some existing current source dir of the current app dir"
    (is (= 1 (count (sut/classpath-dirs built-app-stub))))))
