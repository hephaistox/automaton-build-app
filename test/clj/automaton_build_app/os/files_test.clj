...
(deftest change-extension-test
  ...
  (testing "Filename is changed"
    (is (= "foo.mov"
           (sut/change-extension "foo"
                                 ".mov"))))
  (testing "Relative path is ok"
    (is (= "foo/bar.mov"
           (sut/change-extension "foo/bar.txt"
                                 ".mov"))))
  (testing "Absolute path is ok"
    (is (= "/tmp/foo.mov"
           (sut/change-extension "/tmp/foo.bar"
                                 ".mov")))))
...
(deftest remove-useless-path-elements-test
  ...
  (testing "No path"
    ...
    (is (nil?
           (sut/remove-useless-path-elements nil)))))
...
(deftest create-dir-path-test
  ...
  (testing "Empty string is a relative path"
    (is (= ""
           (sut/create-dir-path)))))
...
(deftest create-absolute-dir-path-test
  (testing "Relative path is built and finish with separator"
    (is (= "/tmp/foo/bar/"
           (sut/create-absolute-dir-path "tmp" "foo" "bar")))))
...
(deftest is-absolute?-test
  (testing "Relative path"
    (is (not (sut/is-absolute? "foo/bar"))))
  (testing "Empty relative path"
    (is (not (sut/is-absolute? ""))))
  (testing "Absolute path"
    (is (sut/is-absolute? "/foo")))
  (testing "Empty absolute path"
    (is (sut/is-absolute? "/"))))
