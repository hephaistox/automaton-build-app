...
(deftest change-extension-test
  ...
  (testing "No extensions means removing the extension" (is (= "" (sut/change-extension "" nil) (sut/change-extension "" ""))))
...
(deftest remove-useless-path-elements-test
  (testing "Remove double separators"
    ...
    (is (= "/User/john-doe/videos/"
           ...
           (sut/remove-useless-path-elements "/////User///john-doe/////videos////"))))
  (testing "Remove current dir as a subdir" (is (= "User/john-doe/videos" (sut/remove-useless-path-elements "User/./john-doe/./videos"))))
...
(deftest create-dir-path-test
  (testing "Relative path is built and finish with separator" (is (= "tmp/foo/bar/" (sut/create-dir-path "tmp" "foo" "bar"))))
  (testing "Empty string is a relative path" (is (= "" (sut/create-dir-path)))))

(deftest create-absolute-dir-path-test
  (testing "Relative path is built and finish with separator" (is (= "/tmp/foo/bar/" (sut/create-absolute-dir-path "tmp" "foo" "bar")))))

(deftest match-extension?-test
  (testing "Check extenstion is matching" (is (sut/match-extension? "foo.bar" ".bar")) (is (sut/match-extension? "foo.bar" ".txt" ".bar")))
...
(deftest file-in-same-dir-test
  (testing "Empty root directory is ok" (is (= "foo" (sut/relativize-to-pwd (sut/file-in-same-dir "" "foo")))))
  ...
  (testing "If the source is file, stored in the same parent"
    (is (= "automaton/automaton-foobar/foo" (sut/relativize-to-pwd (sut/file-in-same-dir "automaton/automaton-foobar/deps.edn" "foo")))))
  (testing "If the source is file does not exist, store in the same parent"
    (is (= "automaton/automaton-foobar/foo"
           (sut/relativize-to-pwd (sut/file-in-same-dir "automaton/automaton-foobar/deps-does-not-exist.edn" "foo"))))))
...
(deftest extract-path-test
  (testing "A relative file returns nil" (is (= "." (sut/relativize-to-pwd (sut/extract-path "README.md")))))
  ...
  (let [tmp-dir (fs/create-temp-dir)]
    (testing "Directory copy"
      (do (sut/copy-files-or-dir [(io/resource "os/resource-test-copy-dir")] tmp-dir)
          (is (= #{"test1" "test2"} (into #{} (map (fn [file] (str (fs/relativize tmp-dir file))) (fs/glob tmp-dir "**"))))))))
  (testing "If files are not a vector of string" (is (not (sut/copy-files-or-dir {} "/tmp"))))
  (let [tmp-dir (fs/create-temp-dir)]
    (testing "File copy"
      (is (= (do (sut/copy-files-or-dir [(io/resource (str "os/resource-test-copy-dir" fs/file-separator "test1"))] tmp-dir) #{"test1"})
...
(deftest directory-exists?-test
  ...
  (testing "An existing directory inside directory is detected" (is (sut/directory-exists? "env/development"))))
...
(deftest filter-to-existing-files-test
  (testing "Non existing files are removed"
    (is (= ["deps.edn" "bb.edn"] (sut/filter-to-existing-files "deps.edn" "non-existing-file" "bb.edn")))))
...
(deftest filter-existing-dir-test
  (testing "Only existing dir are filtered" (is (= 1 (count (sut/filter-existing-dir ["env" "non-existing-dir-filtered"]))))))

(deftest search-in-parents-test
  (testing "Search in current dir is ok" (is (= (sut/absolutize "") (sut/search-in-parents (sut/absolutize "") "bb.edn"))))
  (testing "Search in current dir is ok" (is (= (sut/absolutize "") (sut/search-in-parents (sut/absolutize "src/clj") "bb.edn"))))
  (testing "Non existing file return nil" (is (nil? (sut/search-in-parents "" "non-existing-file")))))
