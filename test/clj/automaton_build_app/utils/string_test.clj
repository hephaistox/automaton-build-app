(ns automaton-build-app.utils.string-test
  (:require [clojure.test :refer [deftest is testing]]
            [automaton-build-app.utils.string :as sut]
            [clojure.string :as str]))

(deftest remove-last-character
  (testing "Remove a character" (is (= "This" (sut/remove-last-character "Thiss"))) (is (= "This" (sut/remove-last-character "Thisù"))))
  (testing "One character is removed" (is (= "" (sut/remove-last-character "T"))) (is (= "" (sut/remove-last-character "ù"))))
  (testing "Empty string is ok" (is (= "" (sut/remove-last-character ""))) (is (= "" (sut/remove-last-character nil)))))
...
(deftest limit-length-test
  (testing "Ellipsis is limited to " (is (= "f..." (sut/limit-length "foobar" 4))) (is (= "fo..." (sut/limit-length "foobar" 5))))
...
(deftest remove-trailing-character-test
  (testing "Remove trailing character if needed"
    (is (= "foo" (sut/remove-trailing-character "foo/" \/) (sut/remove-trailing-character "foo" \/))))
  (testing "Empty strings are ok"
    (is (every? str/blank?
                [(sut/remove-trailing-character "" \/) (sut/remove-trailing-character nil \/) (sut/remove-trailing-character "/" \/)]))))
